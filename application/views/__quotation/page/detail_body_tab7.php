...
<div class="panel panel-default ">
          ...
          <div class="panel-heading" style="padding-bottom:0;">
            ...
            //get job_type from tbt_quotation
              ...
              // $quotation_data['job_type'] = 'ZQT2';
                $ship_to = $this->__ps_project_query->getObj('sap_tbm_ship_to', array('id' => $quotation_data['ship_to_id']));  

                 //=== check is prospect == 
                 if($quotation_data['is_prospect']==1){ 
                    $plant_code = $quotation_data['plan_code_prospect'];
                 }else{
                     $plant_code = $ship_to['plant_code'];
                 }//end else             
                ...
                //cheage margin suport jon special               
                if($quotation_data['is_prospect']==1){
                    //echo $quotation_data['account_group'];
                    $QT_doctype = $quotation_data['job_type'];
                    if($quotation_data['account_group']=='Z16'){
                         $QT_doctype ='ZQT2_extra';
                    }
                }else{
                    $QT_doctype = $quotation_data['job_type'];                    
                }

                $this->db->where('tbm_summary.doc_type', $QT_doctype);
                $this->db->where('tbm_summary.plant_code', $plant_code );
                $query = $this->db->get('tbm_summary');          
                $summary_percent_rate = $query->row_array();     

                 if($quotation_data['is_cal_vat']==0){
                    $summary_percent_rate['percent_vat'] = 0;         
                 }   
                
                if(!empty($summary_data) && ($quotation_data['status'] != "OPEN" && ($quotation_data['status'] != "REJECT")) ){
                  $summary_percent_rate = array(
                    'percent_social_security' => $summary_data['mpercent_social_security'],
                    'percent_operation'       => $summary_data['mpercent_operation'],
                    'percent_margin'          => $summary_data['mpercent_margin'],
                    'percent_buffer'          => $summary_data['mpercent_buffer'],
                    'percent_vat'             => $summary_data['mpercent_vat']
                    );
                ...
                $is_special_one = 0;
                if ($ship_to['ship_to_account_group'] == 'Z16' || $ship_to['ship_to_account_group'] == 'Z26') {
                  $is_special_one = 1;
                  $summary_percent_rate['percent_buffer'] = 0;
                }
                ...
                if (!empty($staff_arr)) {
                  foreach ($staff_arr as $employee_level_id => $level_value) {
                    ...
                    <div class="row wrapper">
                      <?php
                      if ($quotation_data['job_type'] == 'ZQT3') {
                        ?>
                        <div class="row col-sm-5"><?php echo $level_value['title'].' ('.$total_group_staff[$employee_level_id]['total'].' คน)'; ?></div>
                        <div class="col-sm-1 text-center">จำนวนชั่วโมง OT</div>
                        <div class="col-sm-1 text-center" style="width:3%">&nbsp;</div>
                        <div class="col-sm-1 text-center">ค่าล่วงเวลา</div>
                        <div class="col-sm-1 text-center" style="width:3%">&nbsp;</div>
                        <div class="col-sm-1 text-center">จำนวนวันทำงาน</div>
                        <div class="col-sm-1 text-center" style="width:3%">&nbsp;</div>
                        <div class="col-sm-1 text-center">จำนวนคน</div>
                        <div class="col-sm-1 text-center">&nbsp;</div>
                        <div class="col-sm-1 text-center">ค่าแรงรวม</div>
                        <?php
                      } else if ($is_special_one == 1) {
                        ?>
                        <div class="row col-sm-5"><?php echo $level_value['title'].' ('.$total_group_staff[$employee_level_id]['total'].' คน)'; ?></div>
                        <div class="col-sm-1 text-center">ค่าแรง</div>
                        <div class="col-sm-1 text-center" style="width:3%">&nbsp;</div>
                        <div class="col-sm-1 text-center">จำนวนวันทำงาน</div>
                        <div class="col-sm-1 text-center" style="width:3%">&nbsp;</div>
                        <div class="col-sm-1 text-center">จำนวนคน</div>
                        ...
                        <div class="row wrapper">                      
                          ...
                          if ($quotation_data['job_type'] == 'ZQT3') {
                            ...
                            <div class="col-sm-1 text-center">
                              <input type="text" autocomplete="off" class="form-control text-right" value="<?php  echo $subgroup['overtime_hours'];  ?>" disabled>
                            </div>
                            <div class="col-sm-1 text-center" style="width:3%">X</div>
                            <div class="col-sm-1 text-center">
                              <input type="text" autocomplete="off" class="form-control text-right" value="<?php  echo number_format((float)$subgroup['overtime'], 2);  ?>" disabled>
                            </div>
                            <div class="col-sm-1 text-center" style="width:3%">X</div>
                            <div class="col-sm-1 text-center">
                              <input type="text" autocomplete="off" class="form-control text-right" value="<?php  echo $subgroup['work_day'];  ?>" disabled>
                            </div>
                            <div class="col-sm-1 text-center" style="width:3%">X</div>
                            ...
                            <div class="col-sm-1 text-center">=</div>
                            <div class="col-sm-1"><input type="text" autocomplete="off" class="form-control text-right" value="<?php echo number_format($total_subgroup_benefit,2); ?>" disabled></div>
                            <?php
                          } else if ($is_special_one == 1) {
                            ?>
                            <div class="row col-sm-5">
                            ...
                            <div class="col-sm-1 text-center">
                              <input type="text" autocomplete="off" class="form-control text-right" value="<?php if ($subgroup['daily_pay_rate_type'] == 'day') { echo number_format((float)$subgroup['daily_pay_rate'], 2); } else { echo number_format((float)$subgroup['daily_pay_rate']/30, 2); }  ?>" disabled>
                            </div>
                            <div class="col-sm-1 text-center" style="width:3%">X</div>
                            <div class="col-sm-1 text-center">
                              <input type="text" autocomplete="off" class="form-control text-right" value="<?php  echo $subgroup['work_day'];  ?>" disabled>
                            ...
                            <div class="col-sm-1 text-center" style="width:3%">X</div>
                            <div class="col-sm-1 text-center"><input type="text" autocomplete="off" class="form-control text-right" value="<?php echo $subgroup['subgroup_total']; ?>" disabled></div>
                            <div class="col-sm-1 text-center"  style="width:9%">=</div>
                    ...
                    ?>
                    <div class="row wrapper text-right m-r-md">
  ...
  <form id="summary_form" action="<?php echo site_url('__ps_quotation/save_summary/'.$this->quotation_id); ?>" method="post">
    <input type="hidden" name="submit_to_papyrus" value="0">
    <!--################################ end :div detail tool ############################-->
    <?php
    ...
    if (!empty($quotation_data) && ($quotation_data['job_type'] == 'ZQT2' || $quotation_data['job_type'] == 'ZQT3')){
      ...
      <div class="panel panel-default h5" style="margin-bottom: 0;"> 
        ...
        if (!empty($summary_data['mpercent_buffer'])) { 
          $buffer = number_format((float)($summary_data['mpercent_buffer']/100)*$total_8, 2, '.', '');
        } else { 
          ...
          <div class="row wrapper-sm">
            <div class="row col-sm-8">
              ...
              <div class="col-sm-8">
                ...
                <input type="text" autocomplete="off" class="form-control input-s-sm text-center inline percent_buffer" value="<?php echo $summary_percent_rate['percent_buffer']; ?>" <?php if ($quotation_data['job_type'] == 'ZQT2' && $is_special_one == 0) { echo 'readonly'; } ?>> %
              </div>
            </div>
            <div class="col-sm-2 text-center">&nbsp;</div>
            <div class="col-sm-2 text-center">
...
$temp_clerjob = 0;
  ...
  if(!empty($summary_data) && !empty($summary_data['total_variant_price'])) {
    ...
    $vat_variant_val = ($summary_percent_rate['percent_vat']/100)*$summary_data['total_variant_price'];
    $vat_variant = number_format((float)$vat_variant_val, 2, '.', '');
    $total_variant_val = $summary_data['total_variant_price'] + floatval($vat_variant_val); 
    $total_variant = number_format((float)$total_variant_val, 2, '.', '');
  ...
  <div class="panel-body bg-light">
    <div class="wrapper bg-light dker">
      <div class="row wrapper-sm">
        ...
        <div class="col-sm-4 h5">
          ...
          ราคาต่อคน&nbsp;&nbsp;<input type="text" autocomplete="off" class="pull-right form-control text-right bottom_price_per_person" style="width:60%;" value="<?php echo number_format((float)$final_total/$total_subgroup_staff, 2); ?>" disabled>
        </div>
      </div>
      <div class="row wrapper-sm">
        <div class="col-sm-4 text-right h5">
          กำไร
        </div>
        <div class="col-sm-4 text-center h5">
          <input type="text" autocomplete="off" class="form-control text-center input-s-sm inline" readonly value="<?php echo $summary_percent_rate['percent_margin']; ?>">&nbsp;&nbsp;%
        </div>
        <div class="col-sm-4 h5">
          <input type="hidden" name="total_bottom_price" class="total_bottom_price" value="<?php echo $final_total; ?>">
          ราคาเสนอขายสุทธิ&nbsp;&nbsp;<input type="text" autocomplete="off" class="pull-right form-control  total_bottom_price text-right" style="width:60%; " value="<?php  echo number_format((float)$final_total, 2); ?>" disabled>
        </div>
      </div>
      <div class="row wrapper-sm">
        <div class="col-sm-4 text-right h5">
        ...
        <div class="col-sm-4 text-center h5">
          <input type="text" autocomplete="off" class="form-control input-s-sm text-center inline" value="<?php echo $summary_percent_rate['percent_vat']; ?>" readonly>&nbsp;&nbsp;%
          <input type="hidden" name="mpercent_vat" class="" value="<?php echo $summary_percent_rate['percent_vat']; ?>">
        </div>
        <div class="col-sm-4 h5">
          <input type="hidden" name="vat" class="vat vat_input" value="<?php if ($vat_variant_val != 0) { echo $vat_variant_val; } else { echo $vat; } ?>">
          <input type="text" autocomplete="off" style="width:60%; " class="form-control pull-right text-right vat" disabled value="<?php echo number_format((float)$vat, 2); ?>">
        </div>
      </div>      
      <div class="row wrapper-sm">
        <div class="col-sm-4 text-right h5">&nbsp;</div>
        <div class="col-sm-4 text-center h5">&nbsp;</div>
        <div class="col-sm-4 h5">
          <input type="hidden" name="total" class="total total_input" value="<?php if ($total_variant_val != 0) { echo $total_variant_val; } else { if ($quotation_data['job_type'] == 'ZQT1') { echo $total_15; } else if ($quotation_data['job_type'] == 'ZQT3') { echo $total_16; } } ?>">
          ราคาสุทธิหลังบวกภาษี&nbsp;&nbsp;<input type="text" autocomplete="off"  style="width:60%; " class="form-control pull-right  text-right total" disabled value="<?php if ($quotation_data['job_type'] == 'ZQT1') {  echo number_format((float)$total_15, 2); } else if ($quotation_data['job_type'] == 'ZQT3') { echo number_format((float)$total_16, 2);  } ?>">
        </div>
      </div>
    </div>
    <div class="wrapper bg-light dker m-t-sm">
      <div class="row wrapper-sm">
        ...
        <div class="col-sm-4 h5">
          ราคาต่อคน&nbsp;&nbsp;<input type="text" autocomplete="off" name="variant_price_per_person" onkeypress="return isDouble(event)" class="pull-right form-control text-right variant_price_per_person" style="width:60%;" value="<?php if(!empty($summary_data) && !empty($summary_data['total_variant_price'])) { echo number_format((float)$summary_data['total_variant_price']/$total_subgroup_staff, 2); } ?>">
        </div>
      </div>
      <div class="row wrapper-sm">
        <div class="col-sm-4 text-right h5">
         กำไร <?php //echo "test total :".$total_10."<br>".$summary_data['total_variant_price'].' total 8 :'.$total_8.' buffer '.$buffer; echo ''.$total_8; ?>
       </div>
       <div class="col-sm-4 text-center h5">
        <input type="text" name="percent_margin" onkeypress="return isDouble(event)" autocomplete="off" class="form-control text-center input-s-sm inline percent_margin" value="<?php if(!empty($summary_data)){ echo  $summary_data['percent_margin']; }  ?><?php //if(!empty($summary_data) && !empty($summary_data['total_variant_price'])) { echo number_format((float)(($summary_data['total_variant_price']-$total_10)/$total_10)*100, 2, '.', ''); } ?>">&nbsp;&nbsp;%
      </div>
      <div class="col-sm-4 h5">
        ราคาเสนอขายสุทธิ&nbsp;&nbsp;<input type="text" autocomplete="off" name="total_variant_price" onkeypress="return isDouble(event)" class="pull-right form-control text-right total_variant_price" style="width:60%;" value="<?php if(!empty($summary_data) && !empty($summary_data['total_variant_price'])) { echo number_format((float)$summary_data['total_variant_price'], 2); } ?>">
      </div>
    </div>
    <div class="row wrapper-sm">
      <div class="col-sm-4 text-right h5">
      ...
      <div class="col-sm-4 h5">
        <input type="text" autocomplete="off" style="width:60%; " class="form-control pull-right text-right vat_variant" onkeypress="return isDouble(event)" value="<?php echo $vat_variant; ?>">
      </div>
    </div>      
    <div class="row wrapper-sm">
      <div class="col-sm-4 text-right h5">&nbsp;</div>
      <div class="col-sm-4 text-center h5">&nbsp;</div>
      <div class="col-sm-4 h5">
        ราคาสุทธิหลังบวกภาษี&nbsp;&nbsp;<input type="text" autocomplete="off"  style="width:60%; " class="form-control pull-right  text-right total_variant" disabled value="<?php echo $total_variant; ?>">
      </div>
    </div>
  </div>
  <?php
  if (!empty($quotation_data) && ($quotation_data['job_type'] == 'ZQT1')) {
    ...
    <?php
  } else if (!empty($quotation_data) && ($quotation_data['job_type'] == 'ZQT3')) {
    ?>
    <div class="wrapper bg-light dker m-t-sm">
      <?php
      if (!empty($staff_arr)) {
        if(!empty($summary_data) && !empty($summary_data['total_variant_price'])) { 
          $margin = $summary_data['total_variant_price']-$total_10;
        }

        $count = 0; 
        foreach ($staff_arr as $employee_level_id => $level_value) {
          $level_total = ($social_security/$total_subgroup_staff)
          + ($equipment_total/$total_subgroup_staff)
          + ($operation/$total_subgroup_staff)
          + ($margin/$total_subgroup_staff)
          + ($buffer/$total_subgroup_staff)
          + $total_level_share_benefit[$employee_level_id];

          if(!empty($summary_data)) { 
            $level_total += ($summary_data['transportation']/$total_subgroup_staff);
            $level_total += ($summary_data['insurance']/$total_subgroup_staff);
            $level_total -= ($summary_data['maximum_discount']/$total_subgroup_staff);
          }
    ...
    <div class="pull-right m-t-md">      
      <a class="btn btn-default btn-default"><?php echo freetext('cancel'); ?></a>
      <a class="btn btn-default btn-primary margin-left-small" id="save_summary_btn"><?php echo freetext('Save_changes'); ?></a>
    </div>
    <div class="pull-left m-t-md">
      <a class="btn btn-default btn-info" id="submit_to_papyrus" href="#" ><?php echo freetext('submit_quotation_to_papyrus'); ?></a>
    </div>
  </div>
</form>
<?php
  ...
  <section class="panel panel-default">
    ...
    <ul class="list-group no-radius alt">
      <?php if($job_type!='ZQT3' && $acc_gr!='Z16' ){ ?>
       ...
       <!--  <span class="badge bg-info">16</span> -->
        <i class="fa fa-circle icon-muted"></i> 
          <span class="h5 margin-left-medium">ไม่มีข้อมูล น้ำยา,อุปกรณ์</span>
      </li>
       <?php } ?>
       ...
       <li class="list-group-item" href="#">
         <!--  <span class="badge bg-info">16</span> -->
         <i class="fa fa-circle icon-muted"></i> 
         <span class="h5 margin-left-medium">ไม่มีข้อมูล พนักงาน</span>
       </li>
