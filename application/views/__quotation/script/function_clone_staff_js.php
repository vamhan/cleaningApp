...
$("select[name='position_gr"+group_id+"']").attr('disabled', true);
 ...
 $.ajax({
	      type: "GET",
	      url: '<?php echo site_url("__ps_quotation/get_ajax_position");?>'+'/'+level_staff+'/<?php echo $this->quotation_id; ?>' ,
	      data: {},
	      dataType: "json",
	      success: function(data){	        
	        ////console.log('return data is : ');
	         ...
	         //alert('test group');                                  
	           ...
	           console.log(data);       
	            for(var index in data){                          
	              var i = data[index];
	               if(i.id){
	               		var price = (i.price == null) ? 0 : i.price;
	                    obj.append('<option value="'+i.id+'" data-price="'+price+'">'+i.title+'</option>');	                     
	                    //obj.append('<option value="'+i.material_no+'" >'+i.material_description+' '+i.price+'</option>');
	                    count++	                  
	                }
	            }//end for                    
...
$('select.position').on('change', function() {
	...
	var group_div = $(this).closest('.clone_group ');
	var charge_ot_ele = group_div.find('.charge_ot');

    var isint = isInteger(parseFloat(price));


        if (isint) {
          price = parseFloat(price).toMoney( 0 );
        } else {
          var seperator = price.indexOf('.');
          var decimal   = price.substr(seperator+1);
          price = parseFloat(price).toMoney(decimal.length);
        }

	charge_ot_ele.val(price.toString());
});
	...
	$('.level_staff').on('change', function() {
		...
		//alert('level_staff :'+level_staff);
		 ...
		 if(job_type!='ZQT3'){		 
		 	...
		 	$(this).parents(".clone_group").find('.daily_pay_rate').val(0);
		 }
		    ...
		    if(other_type_id=='000000000000030044'){
		     ...
		     if(count_other!=10){
				...
				var row = 	'<div class="col-sm-4 div-other-add">';
			     ...
			     /////////// CALCULATE ALL ////////////////
				$(".daily_pay_rate,.holiday,.work_day,.work_holiday,.sub_group_staff,.input-other,.bonus,.transport_exp,.incentive,.other_value,.rate_position,.special,.total_man,.pay_sunday").on('keyup', function() {
					Calculat_ALL($(this),0);
				});
      ...
      //alert('ts');
         ...
         // alert(cloneGroup);
          ...
          // $("#clone_group_"+cloneGroup+" ").find('.sub_total_group').attr("name","sub_total_group_gr"+ cloneGroup);
           ...
           //== case job_type OT (15july) ==
           //if(job_type=='ZQT3'){
           	$("#clone_group_"+cloneGroup+" ").find('.charge_ot').attr("name","charge_ot_gr"+ cloneGroup);
       	   //}
           ...
           $("#clone_group_"+cloneGroup+" ").find('.pay_sunday').attr("name","pay_sunday_gr"+ cloneGroup);
...
$(".radio_pay_rate").on('click', function() {
	...
	if(job_type!='ZQT3'){
		...
		$(this).parents(".clone_group").find('.overtime_per_hrs').val('');	
	}else{
		$(this).parents(".clone_group").find('.daily_pay_rate').val(0);
		$(this).parents(".clone_group").find('.holiday').val(0);	
		$(this).parents(".clone_group").find('.overtime_per_hrs').val(0);
	}
...
if(daily_pay_rate_type_gr=='day'){
		...
		if(work_holiday_temp=='' || work_holiday_temp==undefined){ work_holiday_temp='0';  }
	...
	var work_holiday_temp = $(this).parents(".clone_group").find('.work_holiday').val();
		if(work_holiday_temp=='' || work_holiday_temp==undefined){ work_holiday_temp='0';  }
		work_holiday_temp =parseFloat(work_holiday_temp);
		//alert('w1:'+work_holiday_temp);

	var daily_pay_rate =  $(this).val();
	if(daily_pay_rate=='' || daily_pay_rate==undefined){ daily_pay_rate='0';  }
	daily_pay_rate = replaceComma(daily_pay_rate);
	daily_pay_rate = parseFloat(daily_pay_rate);
	//alert('d1:'+daily_pay_rate);

	var holiday = 0;
		holiday =parseFloat(holiday);
		holiday = parseFloat((daily_pay_rate/30)*(work_holiday_temp/12));
		//set : 2 double
		holiday = holiday.toFixed(2);
	...
	var overtime_perhrs = 0;
		overtime_perhrs = parseFloat(overtime_perhrs);
		overtime_perhrs = parseFloat((daily_pay_rate/(30*8))*1.5);
		//set : 2 double
		overtime_perhrs = overtime_perhrs.toFixed(2);
		//add function comma
		overtime_perhrs = commaSeparateNumber(overtime_perhrs);
		//alert(holiday);
		$(this).parents(".clone_group").find('.overtime_per_hrs').val(overtime_perhrs);

		Re_calculate_per_person_month($(this));
...
//$(".daily_pay_rate,.work_day,.sub_group_staff,.holiday,.work_holiday")
//$(".daily_pay_rate,.holiday,.work_day,.work_holiday,.sub_group_staff,.input-other,.bonus,.incentive,.other_value,.rate_position,.special,.total_man").on('keyup', function() {
//alert('calculate_all');
...
if(daily_pay_rate_type_gr!=undefined){   
	if(daily_pay_rate_type_gr=='day'){
...
//group_id
	...
	var count_sub_group = ele.parents(".clone_group").find('.count_sub_group').val();
		...
		for (var i = 1; i <= count_sub_group; i++) {
			...
			var overtime_hrs_ele = ele.parents(".clone_group").find("input[name='overtime_hrs_subg"+i+"_gr"+group_id+"']");
			var overtime         = overtime_hrs_ele.closest('.div-overhrs-group').find('.overtime').val();
			if(overtime=='' || overtime==undefined){ overtime=0;  }

			var work_day = ele.parents(".clone_group").find("input[name='work_day_subg"+i+"_gr"+group_id+"']").val();		
			var overtime_hrs = overtime_hrs_ele.val();
...
function Re_calculate_per_person_month(ele){
//alert('Recal month');
//$(".work_holiday").on('keyup', function(){
	var job_type = $('#job_type').val();
//group_id
	...
	// var overtime = ele.parents(".clone_group").find('.overtime').val();	
	var holiday = ele.parents(".clone_group").find('.holiday').val();	
	//alert('holiday_1 ::'+holiday);

	var staff_gruup = ele.parents(".clone_group").find('.total_man').val();
	var pay_sunday = ele.parents(".clone_group").find('.pay_sunday').val(); 

	if(daily_pay_rate=='' || daily_pay_rate==undefined){ daily_pay_rate='0'; }// ele.parents(".clone_group").find('.daily_pay_rate').val(daily_pay_rate);}
	// if(overtime=='' || overtime==undefined){ overtime='0';  }//ele.parents(".clone_group").find('.overtime').val(overtime);}
	if(holiday=='' || holiday==undefined){ holiday='0';  }

	if(staff_gruup=='' || staff_gruup==undefined){ staff_gruup='0';  }

	if(pay_sunday=='' || pay_sunday==undefined){ pay_sunday='0';  }	
	...
	var count_sub_group = ele.parents(".clone_group").find('.count_sub_group').val();
	    ...
	    // overtime = replaceComma(overtime); 
	    holiday = replaceComma(holiday);
	    pay_sunday = replaceComma(pay_sunday);

	    daily_pay_rate = parseFloat(daily_pay_rate);
	    staff_gruup = parseFloat(staff_gruup);
	    //alert(staff_gruup);
	    pay_sunday = parseFloat(pay_sunday);

		/////////////////////////////////////////
	    //====  stap 1  : sarary_staff_total ====
	    /////////////////////////////////////////
	      var temp_sarary_staff_total = 0;
	      temp_sarary_staff_total = parseFloat(temp_sarary_staff_total);
	      temp_sarary_staff_total = parseFloat(daily_pay_rate*staff_gruup);
	      //alert(temp_sarary_staff_total);

	    /////////////////////////////////////////
	    //====  stap 2  : per_person_avg ======= 
	    ////////////////////////////////////////
	    var man_month = 0;
	    for (var p = 1; p <= count_sub_group; p++) {
	         
	        var temp_work_day = ele.parents(".clone_group").find("input[name='work_day_subg"+p+"_gr"+group_id+"']").val();
	        var temp_staff_sub_gr = ele.parents(".clone_group").find("input[name='sub_group_staff_subg"+p+"_gr"+group_id+"']").val();
	        
	        if(temp_work_day=='' || temp_work_day==undefined){ temp_work_day=0; }
	        if(temp_staff_sub_gr=='' || temp_staff_sub_gr==undefined){ temp_staff_sub_gr=0; }   
	        
	        temp_work_day = parseFloat(temp_work_day);
	        temp_staff_sub_gr = parseFloat(temp_staff_sub_gr);

	        // calculate manmoth
	          man_month = parseFloat(man_month);
	          man_month = parseFloat(man_month)+parseFloat(temp_work_day*temp_staff_sub_gr);
	          //alert('man-month :'+man_month);
	    }//end for
	    //alert('final-man-month :'+man_month);		

		//////////////////////////////////////////////////////
	    //====  stap 3  : check holiday and check sunday  ====  
	    //////////////////////////////////////////////////////
	    for (var i = 1; i <= count_sub_group; i++) {
	      ...
	      var per_person =  0;
			if(man_month!=0){
				per_person = parseFloat(per_person);
				per_person = parseFloat(temp_sarary_staff_total/man_month);
				//alert('salary/man-month :'+per_person);
			}else{
				per_person =  parseFloat(0)
				//alert('salary/man-month 0 :'+per_person);
			}	
			//alert('salary/man-month final:'+per_person);

	      var checkedValue = '';
	      var checkedValue_test = ''; 
	      var checked_su = 1;
	      var checked_hol = 0; 
	      var inputElements = ''; 
	      inputElements = ele.parents(".clone_group").find("#clone_sub_group_"+i+" .day-radio")
	      for(var k=0; inputElements[k]; ++k){
	            if(inputElements[k].checked){
	                 checkedValue = inputElements[k].value;
	                 checkedValue_test += ','+inputElements[k].value;               
	                 if(checkedValue=='HOL'){
	                    checked_hol = 1;
	                 }
	                if(checkedValue=='MON' || checkedValue=='TUE' || checkedValue=='WED' || checkedValue=='THU' || checkedValue=='FRI' || checkedValue=='SAT' || checkedValue=='HOL' ){//check sunday
			           		checked_su = 0;
			           }
	                //break;
	            }
	      }
	      // alert(checkedValue_test);
	      // alert('check sun :'+checked_su);
	      // alert('check Hol :'+checked_hol);	  

	      var work_day = ele.parents(".clone_group").find("input[name='work_day_subg"+i+"_gr"+group_id+"']").val();   
	      var overtime_hrs_ele = ele.parents(".clone_group").find("input[name='overtime_hrs_subg"+i+"_gr"+group_id+"']");
	      var overtime_hrs = overtime_hrs_ele.val();
	    
	      var overtime = overtime_hrs_ele.prev().val(); 
	      if(overtime=='' || overtime==undefined){ overtime='0';  }//ele.parents(".clone_group").find('.overtime').val(overtime);}
	      if(work_day=='' || work_day==undefined){ work_day=0; }// ele.parents(".clone_group").find("input[name='work_day_subg"+i+"_gr"+group_id+"']").val(work_day);}
	      //if(overtime_hrs=='' || overtime_hrs==undefined){ overtime_hrs=0; }// ele.parents(".clone_group").find("input[name='overtime_hrs_subg"+i+"_gr"+group_id+"']").val(overtime_hrs);}
	      ...
	      //====== CASE :: SUM WORKDAY ======= 
	      //==================================
	        per_person = parseFloat(per_person)*parseFloat(work_day);
	        //alert('sumper:'+per_person);
	      //=================================
	      //========= CASE :: HOLLIDAY ====== 
	      //=================================
	        // if(job_type=='ZQT1'){
	        //  per_person = parseFloat((daily_pay_rate+(work_day*overtime))+holiday);
	        // }else{
	        //  per_person = parseFloat(((daily_pay_rate/30)*work_day)+(work_day*overtime)+holiday);
	        // }
	        if(checked_hol==1){
	          per_person = parseFloat(per_person)+parseFloat(holiday);
	          //alert('sumhol_per:'+per_person);
	        }

	      //===================================
	      //========= CASE :: OVERTIME ======== 
	      //===================================
	          per_person = parseFloat(per_person)+parseFloat(work_day*overtime);

		    //===========================================
			//========= CASE ::  SUNDAY  ============== 
			//===========================================
				//alert('berfor su per:'+per_person);
				if(checked_su==1){
					per_person = parseFloat(per_person)+parseFloat(pay_sunday*work_day);
					//alert('sun');
				}	        

	      //alert('final per:'+per_person);
	      //set : 2 double
	      per_person = per_person.toFixed(2);
	      //alert('per_person : '+per_person);
	      //set per_person 
...
}//end function Re_calculate_per_person_month


function Calculate_per_person_date(ele,sub_group_id){


//======= start : click onkey_up person daily_pay_rate ====
//$(".daily_pay_rate,.holiday,.work_day,.work_holiday,.sub_group_staff").on('keyup', function() {
...
//});//======= end : click onkey_up person daily_pay_rate ====
}//end function
...
//group_id
	...
	var count_sub_group = ele.parents(".clone_group").find('.count_sub_group').val();
		...
		var temp_sarary_staff_total = 0;
			temp_sarary_staff_total = parseFloat(temp_sarary_staff_total);
			temp_sarary_staff_total = parseFloat(daily_pay_rate*staff_gruup);
			//alert('temp_sarary_staff_total:'+temp_sarary_staff_total);
		...
		for (var i = 1; i <= count_sub_group; i++) {
			...
			//========= CASE ::  SUNDAY  ============== 
			//===========================================
				//alert('berfor su per:'+per_person+' | '+pay_sunday);
				if(checked_su==1){
					per_person = parseFloat(per_person)+parseFloat(pay_sunday*work_day);
					//alert(per_person);
				}
		...
		//////////////////////////////////////////
				...
				Calculat_Per_group_ALL(ele,per_person,i,group_id);
					...
					//== call wage ==
					Calculate_wage_month(ele,sub_group_id);
...
}//end Calculate_per_person_month
...
//$(".daily_pay_rate,.work_day,.sub_group_staff").on('keyup', function() {
//group_id
	var group_id = ele.parents(".clone_group").attr("id");
 	var str = group_id;
    group_id = str.substring(12);
...
// $(".daily_pay_rate,.work_day,.sub_group_staff,.holiday,.work_holiday").on('keyup', function() {
...
function Calculate_wage_month(ele,sub_group_id){
...
//count_sub_group = parseFloat(count_sub_group);
 ...
 // alert('total_per_group :'+total_per_group);
		...
		var waege = 0;
		//waege= parseFloat(waege);
		if(total_per_group ==0 || staff_gruup==0){
			waege = 0;	
		}else{
			waege = parseFloat(total_per_group)/parseFloat(staff_gruup);		
		}//end else		
...
if(daily_pay_rate_type_gr=='day'){
	...
	work_holiday =parseFloat(work_holiday);
	$(this).parents(".clone_group").find('.work_holiday').val(work_holiday);
	//alert('w:'+work_holiday);

	var daily_pay_rate =  $(this).parents(".clone_group").find('.daily_pay_rate').val();	
	if(daily_pay_rate=='' || daily_pay_rate==undefined){ daily_pay_rate='0';  }
	daily_pay_rate = replaceComma(daily_pay_rate);
	daily_pay_rate = parseFloat(daily_pay_rate);
	//alert('d:'+daily_pay_rate);

	var holiday = 0;
	holiday =parseFloat(holiday);
	holiday = parseFloat((daily_pay_rate/30)*(work_holiday/12));
	//set : 2 double
	holiday = holiday.toFixed(2);
...
$(".compress").on('click',function(){ 
	...
	if(compress_type=="compress"){	
		...
		$(this).parents(".box-subgroup").find(".div-day-subgroup").css('margin-bottom', '2px');
		...
		$(this).parents(".box-subgroup").addClass("col-sm-4");
    	...
    	//set div box-subgroup
		$(this).parents(".box-subgroup").removeClass("col-sm-4");
...
var group_id = str.substring(12);
 ...
 //alert('clon group :: '+id_clone_group+' || clone_sub_gr :: clone_sub_group_'+count_sub_group+' || subg :'+count_sub_group+'|| group_id :'+group_id);
  	...
  	var group_div = clone.closest('.clone_group');
	var overtime_per_hrs = group_div.find('.overtime_per_hrs').val();
	if(overtime_per_hrs=='' || overtime_per_hrs==undefined  || overtime_per_hrs == null){ overtime_per_hrs=0; }	
	// console.log(group_div);
	var overtime = 0;
	//if (overtime_per_hrs != 0) {		
		//alert(overtime_per_hrs);
		var overtime_hrs_list = group_div.find('.overtime_hrs');
		overtime_hrs_list.each(function() {
			var val = $(this).val();
			val = (val == undefined || val == null || val == '') ? 0 : val;
			var overtime_ele = $(this).closest('.div-overhrs-group').find('.overtime');
			overtime_ele.val(parseFloat(val)*parseFloat(overtime_per_hrs));
		});
...
// == add 15 july : case :ot == 
//if(job_type=='ZQT3'){
	clone.find('.charge_ot').prop("name","charge_ot_gr"+ count_add_group);
//}
...
clone.find('.pay_sunday').prop("name","pay_sunday_gr"+ count_add_group);
...
var level_staff = clone.find('.level_staff').val();
...
if (clone.find('.level_staff option[value="'+level_staff+'"]').length > 0) {
	clone.find('.level_staff option[value="'+level_staff+'"]').attr('selected', 'selected');
} else {
	clone.find('.level_staff option:first').attr('selected', 'selected');
}

var position = clone.find('.position').val();
...
if (clone.find('.position_clone option[value="'+position+'"]').length > 0) {
	clone.find('.position_clone option[value="'+position+'"]').attr('selected', 'selected');
} else {
	clone.find('.position_clone option:first').attr('selected', 'selected');
}

var uniform = clone.find('.uniform').val();
...
if (clone.find('.uniform option[value="'+uniform+'"]').length > 0) {
	clone.find('.uniform option[value="'+uniform+'"]').attr('selected', 'selected');
} else {
	clone.find('.uniform option:first').attr('selected', 'selected');
}

clone.find('.overtime_per_hrs').after('<input data-parsley-required="true" data-parsley-error-message="" type="text" autocomplete="off" onkeypress="return isDouble(event)" class="form-control overtime_per_hrs_clone text-right" value="'+clone.find('.overtime_per_hrs').val()+'">');
clone.find('.overtime_per_hrs').remove();
clone.find('.overtime_per_hrs_clone').addClass('overtime_per_hrs');
clone.find('.overtime_per_hrs_clone').removeClass('overtime_per_hrs_clone');

clone.find('.daily_pay_rate').after('<input  data-parsley-required="true" data-parsley-error-message="" type="text" autocomplete="off" onkeypress="return isDouble(event)" class="form-control daily_pay_rate_clone text-right" name="daily_pay_rate_gr'+count_add_group+'" value="'+clone.find('.daily_pay_rate').val()+'">');
	   ...
	   //alert('count_sub_group :'+count_sub_group);
		for (var j = 1; j <= count_sub_group; j++) { 
	     ...
	     //chang name sub group
			...
			clone.find('#clone_sub_group_'+j+' .time_end').after('<input data-parsley-required="true" data-parsley-error-message="" type="text" autocomplete="off" style="width:60px;" onkeypress="return isNumberTime(event)" maxlength="5" class="time_end_clone mask-time" name="time_end_subg'+j+'_gr'+count_add_group+'" placeholder="00:00" value="'+clone.find('#clone_sub_group_'+j+' .time_end').val()+'">');
...
///////////////////coopygroup call funcoton //////////////////////////////
		...
		$('.daily_pay_rate,.holiday,.transport_exp,.incentive,.bonus,.rate_position,.special,.input-other,.other_value,.charge_ot').on('keyup', function() {
		...
		$(".work_holiday").on('keyup', function() {
			...
			if(daily_pay_rate_type_gr=='day'){
				...
				work_holiday =parseFloat(work_holiday);
				$(this).parents(".clone_group").find('.work_holiday').val(work_holiday);
				//alert('w2:'+work_holiday);

				var daily_pay_rate =  $(this).parents(".clone_group").find('.daily_pay_rate').val();	
				if(daily_pay_rate=='' || daily_pay_rate==undefined){ daily_pay_rate='0';  }
				daily_pay_rate = replaceComma(daily_pay_rate);
				daily_pay_rate = parseFloat(daily_pay_rate);
				//alert('d2:'+daily_pay_rate);
...
$('.overtime_per_hrs,.daily_pay_rate').on('keyup', function(){
	var group_div = $(this).closest('.clone_group ');
	var overtime_per_hrs = $(this).val();
	if(overtime_per_hrs=='' || overtime_per_hrs==undefined  || overtime_per_hrs == null){ overtime_per_hrs=0; }
	// console.log(group_div);
	var overtime = 0;
	//if (overtime_per_hrs != 0) {
		...
		overtime_hrs_list.each(function() {
			...
			//overtime_ele.val(parseFloat(val)*overtime_per_hrs);
			overtime_ele.val(parseFloat(val)*parseFloat(overtime_per_hrs));
		});
	//}
...
$('.overtime_hrs,.daily_pay_rate').on('keyup', function(){
	var group_div = $(this).closest('.clone_group ');
	var overtime_per_hrs = group_div.find('.overtime_per_hrs').val();
	if(overtime_per_hrs=='' || overtime_per_hrs==undefined  || overtime_per_hrs == null){ overtime_per_hrs=0; }		
	// console.log(group_div);
